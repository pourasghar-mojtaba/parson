...
<script>
    ...
    function updateComment(id) {
        $.ajax({
            url: '{{ route("blogcomment.update",$blogcomment->id) }}',
            type: 'put',
            cache: false,
            data: {"_token": "{{ csrf_token() }}", "comment": $('#edit_comment').val()},
            datatype: 'json',
            beforeSend: function () {
            },
            success: function (data) {
                if (data.success) {
                    ...
                    $('.blog_comment_'+id).text(data.comment);
