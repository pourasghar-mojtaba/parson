...
@section('description','')
@section('open_graph')
    <meta property='og:site_name' content='{{ $organization->title }}'>
...
@section('content')
                    ...
                    <div
                        ...
                        <div class="category-content-box">
                            <p class="category-content-text show-more-content">
                                {!! $organization->present()->descriptionHtml  !!}
                            </p>
            ...
            <div class="kethab-category-container container">
                    ...
                    <div class="filter-panels-main">
                        ...
                        <div class="col-12">
                            ...
                            <div class="filter-panel-box">
                                ...
                                <div class="filter-option-box filter-option-scroll scrollbar-inner" id="translator_checks">
                ...
                <div class="container kethab-last-news-container ">
                    <div class="library-row-title col-12">
                        <h2 class="library-title">اخبار {{ $organization->title }}</h2>
    ...
    <script>
        $('.btn-follow').click(function () {
            follow('{{ route("follower.follow") }}', "{{ csrf_token() }}", 0, '{{ $organization->id }}', 0, 0);
        });

        function getFilterBooks() {
            var url = '{{ route("organization.getbooks", [":organization_id"]) }}';
            url = url.replace(':organization_id', '{{ $organization->id }}');
            openLoading();
            $.ajax({
    ...
    <script>
        var category_url = '{{ route("organization.getcategories", [":organization_id"]) }}';
        category_url = category_url.replace(':organization_id', '{{ $organization->id }}');
        ...
        var writer_url = '{{ route("organization.getpersons", [":organization_id",":person_role_id"]) }}';
        writer_url = writer_url.replace(':organization_id', '{{ $organization->id }}');
        ...
        var translator_url = '{{ route("organization.getpersons", [":organization_id",":person_role_id"]) }}';
        translator_url = translator_url.replace(':organization_id', '{{ $organization->id }}');
